...
TEST(Visit, MutVarMutType) {
  ...
  // Check qualifier.
  EXPECT_EQ(LRef, mpark::unsafe::visit(get_qual(), v));
  EXPECT_EQ(RRef, mpark::unsafe::visit(get_qual(), lib::move(v)));
}
...
TEST(Visit, MutVarConstType) {
  ...
  // Check qualifier.
  EXPECT_EQ(ConstLRef, mpark::unsafe::visit(get_qual(), v));
  EXPECT_EQ(ConstRRef, mpark::unsafe::visit(get_qual(), lib::move(v)));
}
...
TEST(Visit, ConstVarMutType) {
  ...
  /* constexpr */ {
    ...
    // Check qualifier.
    static_assert(ConstLRef == mpark::unsafe::visit(get_qual(), cv), "");
    static_assert(ConstRRef == mpark::unsafe::visit(get_qual(), lib::move(cv)),
                  "");
  }
#endif
}
...
TEST(Visit, Zero) { EXPECT_EQ("", mpark::unsafe::visit(concat{})); }

TEST(Visit_Homogeneous, Double) {
  mpark::variant<int, std::string> v("hello"), w("world!");
  EXPECT_EQ("helloworld!", mpark::unsafe::visit(concat{}, v, w));
  ...
  /* constexpr */ {
    ...
    static_assert(303 == mpark::unsafe::visit(add_ints{}, cv, cw), "");
    static_assert(202 == mpark::unsafe::visit(add_ints{}, cw, cx), "");
    static_assert(101 == mpark::unsafe::visit(add_ints{}, cx, cv), "");
    static_assert(0 == mpark::unsafe::visit(add_ints{}, cx, cx), "");
  }
#endif
}
...
TEST(Visit_Homogeneous, Quintuple) {
  ...
  EXPECT_EQ("101+202=303", mpark::unsafe::visit(concat{}, v, w, x, y, z));
}
