  ...
  namespace detail {
    ...
    namespace visitation {
      ...
      struct base {
        ...
        template <typename R>
        struct dispatcher<R, false> {
          template <std::size_t B, typename... ITs, typename F, typename... Vs>
          [[noreturn]] static R dispatch(F &&, typename ITs::type &&..., Vs &&...) {
            MPARK_BUILTIN_UNREACHABLE;
          }
        ...
        template <typename R>
        struct dispatcher<R, true> {
          template <std::size_t B, typename... ITs, typename F>
...
#define MPARK_DISPATCH(I)                                    \
  dispatcher<R, (I < lib::decay_t<V>::size())>::             \
      template dispatch<0, ITs..., lib::indexed_type<I, V>>( \
          lib::forward<F>(f),                                \
          lib::forward<typename ITs::type>(visited_vs)...,   \
          lib::forward<V>(v),                                \
          lib::forward<Vs>(vs)...)
...
#define MPARK_DEFAULT(I)                                                      \
  dispatcher<R, (I < lib::decay_t<V>::size())>::template dispatch<I, ITs...>( \
      lib::forward<F>(f),                                                     \
      lib::forward<typename ITs::type>(visited_vs)...,                        \
      lib::forward<V>(v),                                                     \
      lib::forward<Vs>(vs)...)
...
#define MPARK_DISPATCH_AT(I)                                               \
  dispatcher<R, (I < lib::decay_t<V>::size())>::template dispatch_case<I>( \
      lib::forward<F>(f), lib::forward<V>(v), lib::forward<Vs>(vs)...)

#define MPARK_DEFAULT(I)                                                 \
  dispatcher<R, (I < lib::decay_t<V>::size())>::template dispatch_at<I>( \
          ...
          DECLTYPE_AUTO_RETURN(
              base::dispatcher<
                  decltype(lib::invoke(
                      ...
                      access::base::get_alt<0>(lib::forward<Vs>(vs))...)),
                  true>::template dispatch<0>(lib::forward<Visitor>(visitor),
                                              lib::forward<Vs>(vs)...))
          ...
          DECLTYPE_AUTO_RETURN(
              base::dispatcher<
                  decltype(lib::invoke(
                      lib::forward<Visitor>(visitor),
                      access::base::get_alt<0>(lib::forward<Vs>(vs))...)),
                  true>::template dispatch_at<0>(index,
                                                 lib::forward<Visitor>(visitor),
                                                 lib::forward<Vs>(vs)...))
      };
